...
export default function RegisterPage() {
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col justify-center px-6 py-12">
      <h1 className="font-display text-3xl italic">{t("auth.joinTitle")}</h1>
      <p className="mt-2 text-sm text-ink/60">{t("auth.registerSubtitle")}</p>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-4">
        ...
        <button type="submit" disabled={loading} className="btn-primary w-full">
          {loading ? t("auth.creatingAccount") : t("footer.createAccount")}
        </button>
      </form>
      ...
      <p className="mt-6 text-center text-sm text-ink/60">
        {t("auth.alreadyHaveAccount")} <Link href="/login" className="text-plum hover:underline">{t("common.signIn")}</Link>
      </p>
    </div>
  );
}
